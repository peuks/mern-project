...
const getClientByIdAndCfid = async (clientID) => {
    ...
    console.log("MERDE", process.env.CFID_CLIENT_ID)
    const data = new FormData();
    data.append('io_mode', 'json');
...
const getClientrByMail = async (clientEmail) => {
    const data = new FormData();
    data.append('io_mode', 'json');
    data.append('do_in', `{"method":"Peoples.getList","params":{"search": {"contains": "${clientEmail}"}}}`);
    const timestamp = + new Date()
    const config = {
        method: 'post',
        url: 'https://apifeed.sellsy.com/0/',
...
const getClientByProspectId = async (prospectId, full = false) => {
    const data = new FormData();
    data.append('io_mode', 'json');
    data.append('do_in', `{"method":"Prospects.getOne","params":{"id": "${prospectId}"}}`);
    const timestamp = + new Date()
    const config = {
        method: 'post',
        url: 'https://apifeed.sellsy.com/0/',
